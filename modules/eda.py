...
def chart_sankey_dinamico(
    ...
    Mostra Motivo1 → Motivo2 → ... → 🔴 CHURN (TARGET_CHURN=1) ou 🟢 NÃO CHURN (TARGET_CHURN=0).
    Os nós finais refletem o valor real de TARGET_CHURN do dataset:
      TARGET_CHURN = 1 → cliente que SAIU da empresa (CHURN confirmado).
      TARGET_CHURN = 0 → cliente com jornada ativa, ainda SEM churn registrado.
    """
    # Suportar tanto MACRO_MOTIVO quanto PERFIL_RECLAMACAO
    motivo_col = None
    if "PERFIL_RECLAMACAO" in df_classified.columns:
    ...
    # ── Nó final por cliente: TARGET_CHURN real do CSV ───────────────────────
    # TARGET_CHURN = 1 → 🔴 CHURN   (cliente saiu da empresa)
    # TARGET_CHURN = 0 → 🟢 NÃO CHURN (cliente ainda ativo)
    if "TARGET_CHURN" in df_f.columns:
        target_per_client = df_f.groupby("ID_CLIENTE")["TARGET_CHURN"].first()
        def _final_node(cid):
            return "🔴 CHURN" if int(target_per_client.get(cid, 0)) == 1 else "🟢 NÃO CHURN"
    else:
        def _final_node(cid):
            return "⛔ CHURN"
    ...
    # Identificar última linha de cada cliente
    last_set = set(df_sorted.groupby("ID_CLIENTE")["POS"].idxmax())

    def _src(r):
        return f"{r[motivo_col]} [{int(r['POS'])+1}]"

    def _tgt(r):
        if r.name in last_set or pd.isna(r["NEXT_MOTIVO"]):
            return _final_node(r["ID_CLIENTE"])
        return f"{r['NEXT_MOTIVO']} [{int(r['POS'])+2}]"

    df_sorted["SRC_NODE"] = df_sorted.apply(_src, axis=1)
    df_sorted["TGT_NODE"] = df_sorted.apply(_tgt, axis=1)
    ...
    # Cores dos nós
    def _node_color(n):
        if n == "🔴 CHURN":    return COLORS["primary"]   # vermelho
        if n == "🟢 NÃO CHURN": return "#2A9D8F"           # verde
        if "⛔" in n:           return COLORS["primary"]   # fallback vermelho
        if "[1]" in n:         return COLORS["accent"]    # laranja (etapa 1)
        return COLORS["secondary"]                        # azul (etapas intermediárias)

    node_colors = [_node_color(n) for n in all_nodes]

    # Cores dos links  (vermelho → CHURN, verde → NÃO CHURN, azul → intermediário)
    def _link_color(tgt):
        if tgt == "🔴 CHURN":    return "rgba(230,57,70,0.30)"
        if tgt == "🟢 NÃO CHURN": return "rgba(42,157,143,0.30)"
        return "rgba(69,123,157,0.25)"

    link_colors = [_link_color(t) for t in flows["TGT_NODE"]]
    ...
    return update_fig_layout(
        fig,
        "🌊 Fluxo da Jornada — Motivo 1 → ... → 🔴 CHURN (TARGET=1) / 🟢 NÃO CHURN (TARGET=0)"
    )
