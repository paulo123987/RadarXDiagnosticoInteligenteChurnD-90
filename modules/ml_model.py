...
def build_features(df_d90: pd.DataFrame, df_classified: pd.DataFrame = None,
    ...
    Parâmetros
    ----------
    df_d90      : DataFrame consolidado D-90 (um cliente por linha).
    df_classified: DataFrame com classificações de agentes (opcional).
    df_raw      : DataFrame original do CSV — usado para ler TARGET_CHURN real.
                  TARGET_CHURN = 0 → clientes com jornada ativa sem churn registrado.
                  TARGET_CHURN = 1 → clientes que saíram da empresa (churn confirmado).
    """
    ...
    # ── TARGET_CHURN: usar valor real do CSV (LGPD: NOME_CLIENTE excluído) ──────
    # TARGET_CHURN = 0 → cliente com jornada ativa que ainda NÃO teve churn
    # TARGET_CHURN = 1 → cliente que saiu da empresa (churn confirmado)
    if df_raw is not None and "TARGET_CHURN" in df_raw.columns:
        target_map = (
            df_raw.groupby("ID_CLIENTE")["TARGET_CHURN"]
            .first()
            .reset_index()
        )
        feat = feat.merge(target_map, on="ID_CLIENTE", how="left")
        feat["TARGET_CHURN"] = feat["TARGET_CHURN"].fillna(0).astype(int)
    else:
        # Fallback: proxy por percentil (usado apenas se df_raw não estiver disponível)
...
def build_features_expanded(df: pd.DataFrame, df_d90: pd.DataFrame) -> pd.DataFrame:
    ...
    # TARGET_CHURN = 0 → cliente com jornada ativa que ainda NÃO teve churn
    # TARGET_CHURN = 1 → cliente que saiu da empresa (churn confirmado)
    # df já é o dataset original que contém TARGET_CHURN por transação;
    # pegamos o valor por ID_CLIENTE (todos os registros do cliente têm o mesmo valor).
    if "TARGET_CHURN" in df_work.columns:
        target_map = (
            df_work.groupby("ID_CLIENTE")["TARGET_CHURN"]
            .first()
            .reset_index()
        )
        feat = feat.merge(target_map, on="ID_CLIENTE", how="left")
        ...
        # Fallback: proxy por percentil (usado apenas se TARGET_CHURN não estiver no df)
        threshold = feat["SCORE_INTENSIDADE"].quantile(0.4)
        feat["TARGET_CHURN"] = (feat["SCORE_INTENSIDADE"] >= threshold).astype(int)
...
def generate_root_cause_diagnosis(feat: pd.DataFrame, results: dict) -> dict:
    ...
    # ── Composição dos dois públicos de TARGET_CHURN ─────────────────────────
    n_sem_churn = int((feat["TARGET_CHURN"] == 0).sum()) if "TARGET_CHURN" in feat.columns else 0
    n_com_churn = int((feat["TARGET_CHURN"] == 1).sum()) if "TARGET_CHURN" in feat.columns else total

    narrativa = (
        f"**Modelo utilizado:** {best_model_name}\n\n"
        f"---\n"
        f"### 👥 Composição da Base Analisada\n"
        f"| Público | Definição | Clientes |\n"
        f"|---------|-----------|---------|\n"
        f"| 🟢 **TARGET\_CHURN = 0** | Clientes com jornada ativa — **ainda sem churn registrado** | **{n_sem_churn}** |\n"
        f"| 🔴 **TARGET\_CHURN = 1** | Clientes que **saíram da empresa** (churn confirmado) | **{n_com_churn}** |\n\n"
        f"---\n"
        f"### 🎯 Score de Risco Preditivo\n"
        f"De um total de **{total} clientes pontuados pelo modelo**, "
        f"**{n_alto} ({n_alto/max(total,1):.0%})** apresentam score de risco **alto** "
        f"e **{n_medio} ({n_medio/max(total,1):.0%})** score de risco **médio**.\n\n"
        ...
        f"A análise diferencia os clientes que **ainda podem ser retidos** (TARGET\_CHURN = 0) "
        f"daqueles que **já realizaram a saída** (TARGET\_CHURN = 1), permitindo ações distintas "
        f"de retenção preventiva e pós-churn."
    )
    ...
    return {
        ...
        "n_sem_churn": n_sem_churn,
        "n_com_churn": n_com_churn,
    }
